...
문자열을 출력한다. 이 안에 값을 넣을 수도 있다. 값은 constants, ◊|var|s, or more complicated ◊|exp|s 를 집어넣을 수 있고, 갯수에 제한은 없다.

◊form{
  printf ( string, ◊|exp|1, ◊|exp|2, ... );
}

문자열 안에서 ◊|val|로 넣는 값은 ◊|conv-spec|s로 표시하고, 그 외 일반 문자는 그대로 집어넣는다. ◊|conv-spec|은 %로 시작한다. % 뒤에 나오는 문자가 어떤 타입의 값이 그 자리에 올지를 나타낸다. 출력할 때 이 문자들은 각각이 가리키는 값으로 바뀌어 출력된다.
...
◊codeblock{
  int i;
	...
	printf("the ◊|val| of i is %d. whereas the ◊|val| of x is %f.", i, x);
}

int type 값은 %d, float type 값은 %f를 각각 ◊|conv-spec|으로 쓴다. 또 문자열 뒤에 나오는 ◊|exp|은 다른 문자를 붙이지 않고 그 이름 그대로를 쓴다.
...
m, p are ◊uncertain{integer constant}, X is a ◊uncertain{letter}.

m과 p 모두 필수가 아니다. p를 제할 경우, p 바로 앞의 .도 같이 제한다.
...
. 뒤에 나오는 p(precision)은 X(◊|conv-spec|)에 따라 뜻이 달라진다:
...
◊item{g는 floating-point 수를 지수나 고정 소수점 형식으로 나타낸다. 그리고 (컴파일러 기준) 적당한 곳에서 어떤 형식으로 출력할지를 정한다. p는 출력할 ◊uncertain{유효 숫자}의 최대 갯수다. f와는 달리 0으로 메꾸는 건 하지 않고, 소수점 뒤 숫자가 없을 경우 소수점을 표시 안한다.}
...
◊|conv-spec| 갯수와 입력 ◊|var| 갯수를 맞춰줘야 한다. 또한 scanf 함수를 쓸 때 ◊|exp|은 앞에 &를 써줘야 한다. 안그러면 알 수 없는 값이 들어가버린다.
